...
### Clients Used:
- **Client ID 2** - Client 2 (Cases: 199, 202, 206, 211)
- **Client ID 3** - Client 3 (Cases: 200, 203, 208, 213)
- **Client ID 5** - Client 5 (Cases: 201, 207, 212)
- **Client ID 6** - Client 6 (Cases: 204, 209)
- **Client ID 7** - Client 7 (Cases: 205, 210)
...
- Case 209: device_id = 53 (Ivoclar Press 1)
...
```
**Expected:** If the default date range includes Oct 2025, cases 199-209 and 211-213 (14 cases)
...
```
**Expected:** Cases 199, 202, 206, 211 (Client: Client 2)
...
```
**Expected:** All except case 204 (50 JOD) and case 211 (no invoice) - 13 cases

#### TC-09: Amount Range (To Only)
```
http://localhost:8000/reports/master?generate_report=1&amount_to=500
```
**Expected:** All except case 205 (900 JOD) and case 211 (no invoice) - 13 cases

#### TC-10: Amount Range (Between)
```
http://localhost:8000/reports/master?generate_report=1&amount_from=100&amount_to=500
```
**Expected:** Cases 199, 200, 201, 202, 203, 206, 207, 208, 209, 210, 212, 213 (12 cases)
**Excluded:** amount below 100, amount above 500, or no invoice
...
```
**Expected:** Cases 200 (3 units), 207 (3 units), 209 (2 units)
...
```
http://localhost:8000/reports/master?generate_report=1&employee_filters%5B0%5D%5Bstage%5D=assignee&employee_filters%5B0%5D%5Bemployee%5D=10
```
**Note:** Admin user ID is 10
**Expected:** All 15 cases (all use admin as assignee)

#### TC-16: Employee Filter (Delivery)
```
http://localhost:8000/reports/master?generate_report=1&employee_filters%5B0%5D%5Bstage%5D=delivery&employee_filters%5B0%5D%5Bemployee%5D=11
```
**Note:** Delivery user ID is 11
...
```
http://localhost:8000/reports/master?generate_report=1&device_filters%5B0%5D%5Btype%5D=sinter&device_filters%5B0%5D%5Bdevice%5D=53
```
**Expected:** Case 209 when device=53 (Ivoclar Press 1)
...
```
http://localhost:8000/reports/master?generate_report=1&from=2025-10-01&to=2025-10-29&doctor%5B%5D=2&material%5B%5D=1&status%5B%5D=1&amount_from=50&units_to=5&show_completed=in_progress&employee_filters%5B0%5D%5Bstage%5D=assignee&employee_filters%5B0%5D%5Bemployee%5D=10
```
**Expected:** No cases (case 211 has no invoice, fails amount_from=50)
...
- Cases: 199, 201, 202, 203, 204, 206, 208, 212, 213 (9 cases)
...
```
**Expected:** Cases 199, 202, 203, 204, 206, 207, 208, 210, 211, 212, 213
...
```
**Expected:** No cases (all in-progress cases are before 2025-10-22)
...
| 201 | 5 | 200 | -1 | 1 | Completed | Implant job type |
...
| 209 | 6 | 220 | 3 | 2 | In-Progress | Has device_id=53 |
| 210 | 7 | 160 | -1 | 1 | Completed | Previous month |
...
2. ✅ Admin user ID is 10
3. ✅ Delivery user ID is 11
