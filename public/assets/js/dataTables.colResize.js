    ...
    $.extend( ColResize.prototype, {
        ...
        */
        ...
        buildDom: function() {
            ...
            that._table.before(that._container); // Insert container now, before deferring other calls
            ...
            // DEFER building columns and other layout-dependent operations
            // This allows the browser to apply the width changes before positioning handlers
            setTimeout(function() {
        ...
        buildColDoms: function() {
            ...
            for (var i = 0, l = $ths.length; i < l; i++) {
                ...
                $col.css({
                    left: Math.ceil($th.offset().left + thWidth - that._container.offset().left)
                });
        ...
        onMouseDown: function() {
            ...
            return function(e) {
                var $col = $(this),
                    $th = $col.data(that.DATA_TAG_ITEM), // Get the associated <th>
                    startX = e.clientX, // Capture starting mouse X position
                    initialThWidth = $th.outerWidth(); // Capture initial <th> width

                var mouseMoveHandler = function(event) {
                    that.onMouseMove(event, $col, startX, initialThWidth); // Pass these values
                };
                var mouseUpHandler = function(upEvent) {
                    that._isDragging = false;
                    // Crucially, remove both handlers from the document
                    $(document).off("mousemove", mouseMoveHandler);
                    $(document).off("mouseup", mouseUpHandler);

                    // Force cleanup cursor and any lingering states
                    $('*').css('cursor', '');
