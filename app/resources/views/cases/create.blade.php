    ...
    <div class="card">
        ...
        <div class="row">
            ...
            <div class="col-md-4  col-xs-6 col-l-4  col-xl-3">
                ...
                <div class="col-md-12 col-xs-12">
                    <select class="select selectpicker greyBG" name="tags[]" data-container="body" multiple data-mdb-placeholder="Tags">
                        @foreach ($tags as $tag)
...
@push('js')
    ...
    container: 'body'
        ...
        // Define the function globally so it can be called from window.load
        function initializeSelectPicker() {
            console.log('Initializing selectpicker...');

            try {
                if (typeof $.fn.selectpicker !== 'undefined') {
                    console.log('Bootstrap Select is available');
                    ...
                    // Initialize selectpickers with error handling
                    $('.selectpicker').each(function() {
                        try {
                            $(this).selectpicker({
    container: 'body'
});
                            console.log('Initialized selectpicker for:', this);
                        } catch (e) {
                            console.warn('Failed to initialize selectpicker:', e);
                            // Don't remove selectpicker class - just add form-control
                            $(this).addClass('form-control');
                        }
                    });

                    // Refresh working selectpickers
                    try {
                        $('.selectpicker').selectpicker('refresh');
                        console.log('Refreshed all selectpickers');
                    } catch (e) {
                        console.warn('Failed to refresh selectpickers:', e);
                    }

                } else {
                    console.warn('Bootstrap Select not loaded - adding form-control class');
                    $('.selectpicker').addClass('form-control');
                }
            } catch (error) {
                console.error('SelectPicker initialization error:', error);
                // Fallback: make all selects work as regular form controls
                $('.selectpicker').addClass('form-control');
            }
        }

        // Wait for ALL scripts to load including Bootstrap Select before initializing
        $(window).on('load', function() {
            // Give additional time for Bootstrap Select to fully load
            setTimeout(function() {
