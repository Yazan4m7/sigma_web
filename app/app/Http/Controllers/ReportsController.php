...
class ReportsController extends Controller
{
    ...
    public function numOfUnitsReport(Request $request)
    {
        ...
        if (isset($request->material) && in_array('all', $request->material)) {
            $selectedMaterials = $materials->pluck('id')->toArray();
        } else {
            // Default to first 4 materials on initial load (when no material filter is set)
            $selectedMaterials = $request->material ?? $materials->take(4)->pluck('id')->toArray();
        }
    ...
    public function repeatsReport(Request $request)
    {
        ...
        $countOrPercentage = $request->countOrPercentageToggle == '1';
...
//        dd($labelToLookFor);
    ...
    public function masterReport(Request $request)
    {
        ...
        $employeesByStage = [
            'design' => \App\User::whereHas('permissions', function($q) {
                $q->where('permission_id', 1);
            })->orWhere('is_admin', 1)->get(['id', 'first_name', 'last_name']),

            'milling' => \App\User::whereHas('permissions', function($q) {
                $q->where('permission_id', 2);
            })->orWhere('is_admin', 1)->get(['id', 'first_name', 'last_name']),

            'printing' => \App\User::whereHas('permissions', function($q) {
                $q->where('permission_id', 3);
            })->orWhere('is_admin', 1)->get(['id', 'first_name', 'last_name']),

            'sintering' => \App\User::whereHas('permissions', function($q) {
                $q->where('permission_id', 4);
            })->orWhere('is_admin', 1)->get(['id', 'first_name', 'last_name']),

            'pressing' => \App\User::whereHas('permissions', function($q) {
                $q->where('permission_id', 5);
            })->orWhere('is_admin', 1)->get(['id', 'first_name', 'last_name']),

            'finishing' => \App\User::whereHas('permissions', function($q) {
                $q->where('permission_id', 6);
            })->orWhere('is_admin', 1)->get(['id', 'first_name', 'last_name']),

            'qc' => \App\User::whereHas('permissions', function($q) {
                $q->where('permission_id', 7);
            })->orWhere('is_admin', 1)->get(['id', 'first_name', 'last_name']),

            'delivery' => \App\User::whereHas('permissions', function($q) {
        ...
        // Apply basic filters
        if ($request->filled('doctor') && !in_array('all', $request->doctor)) {
            $query->whereIn('doctor_id', $request->doctor);
        }

        if ($request->filled('material') && !in_array('all', $request->material)) {
            $query->whereHas('jobs', function($q) use ($request) {
                $q->whereIn('material_id', $request->material);
            });
        }

        if ($request->filled('job_type') && !in_array('all', $request->job_type)) {
            $query->whereHas('jobs', function($q) use ($request) {
                $q->whereIn('type', $request->job_type);
            });
        }

        // Material Type filter (depends on material selection)
        if ($request->filled('material_type') && !in_array('all', $request->material_type)) {
            $query->whereHas('jobs.material.types', function($q) use ($request) {
                $q->whereIn('material_types.id', $request->material_type);
            });
        }

        // Failure Type filter
        if ($request->filled('failure_type') && !in_array('all', $request->failure_type)) {
            $query->whereHas('failureLogs', function($q) use ($request) {
                $q->whereIn('cause_id', $request->failure_type);
            });
        }

        // Abutments filter
        if ($request->filled('abutments') && !in_array('all', $request->abutments)) {
            $query->whereHas('jobs', function($q) use ($request) {
                $q->whereIn('abutment', $request->abutments);
            });
        }

        // Implants filter
        if ($request->filled('implants') && !in_array('all', $request->implants)) {
            $query->whereHas('jobs', function($q) use ($request) {
                $q->whereIn('implant', $request->implants);
            });
        }
        ...
        if ($request->filled('status') && !in_array('all', $request->status)) {
            $stages = array_filter($request->status, function($status) {
        ...
        if ($request->filled('device_filters')) {
            foreach ($request->device_filters as $filter) {
                if (isset($filter['type']) && isset($filter['device'])) {
                    ...
                    // Each build has device_id or printer_id
                            ...
                            default:
                                ...
                                $q->where(function($jobQ) use ($deviceId) {
                                    ...
                                    ->orWhereHas('millingBuild', function($buildQ) use ($deviceId) {
                                        $buildQ->where('device_id', $deviceId)->orWhere('printer_id', $deviceId);
                                    })
