        ...
        <div class="sigma-workflow-body">
            ...
            @if ($type != 'sintering')
                <div class="row g-2 sigma-inputs-container">
                    <div class="sigma-form-group col-6 sigma-input-col"
                        style="display: flex; gap: 10px; align-items: end;">
                        <input id="sigma-build-name-{{ $type }}" class="sigma-form-control"
                    ...
                    @if (in_array($type, ['milling', 'pressing', '3dprinting']))
                        <div class="sigma-form-group col-6 sigma-input-col" style="flex: 1;">
                            <select id="sigma-material-type-{{ $type }}" class="sigma-form-control"
...
<form id="hidden-form-{{ $type }}" method="POST" action="{{ $stageSpecs[$type]['route'] }}" class="d-none">
    ...
    <input type="hidden" name="WaitingPopupCheckBoxes{{ $type }}[]" id="case-ids-{{ $type }}"
        value="">
...
<script>
    ...
    function setInnerTab(btnElement) {
        ...
        $(`[aria-labelledby^='active-${stageKey}'], [aria-labelledby^='waiting-${stageKey}']`).attr('hidden', true)
            .removeClass('active');
